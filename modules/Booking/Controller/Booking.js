 ...
 module.exports = async(req,res)=>{
    ...
    debugger
        ...
        if(new Date(DateFrom) < new Date() || new Date(DateFrom) > new Date(DateTo))
            res.status(400).send("Date from not smaller than Date To or Date From not larger than Current Date");
        //get overlaped rents, (StartA <= EndB) and (EndA >= StartB)
        else if(overlappedRent.length > 0)
            res.status(400).send(`Vehicle already rentet in this period, From 
            ${overlappedRent[0].DateFrom.toISOString()} TO ${overlappedRent[0].DateTo.toISOString()}`); 
        else{
